...
function c2150025.t(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		return re:GetHandler():IsSetCard(0x220) and re:GetLabel()&2>0 and e:GetHandler():IsCanBeSpecialSummoned(e,0,tp,false,false) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and (e:GetHandler():IsLocation(LOCATION_HAND) or e:GetHandler():GetTurnID()<Duel.GetTurnCount())
	end
